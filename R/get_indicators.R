...
get_indicators <- function(bio, cat, lookup,
  ...
  ind$Nppbio <- .sum(results, "IsPrimaryProducer", "Biomass")
...
# Working here.
  ...
  # Mean vulnerability
  ind$mivfishbio <- with(results[results$IsTarget == 1 & results$IsFish, ],
    tapply(Biomass * vulnerability, Time, sum, na.rm = TRUE)) /
    ifelse(ind$Fishtarbio <= 0, NA, ind$Fishtarbio)
  if (sum(ind$Mivfishbio, na.rm = TRUE) == 0) ind$Mivfishbio <- NA

  ind$mivfishcat <- with(results[results$IsTarget == 1 & results$IsFish, ],
    tapply(Catch * vulnerability, Time, sum, na.rm = TRUE)) /
    ifelse(ind$Fishtarcat <= 0, NA, ind$Fishtarcat)
  ...
  ind$ndesbio <- calc_ndes(results[results$IsTarget == 1, ])
  if (sum(results$IsTarget == 1 & results$IsSurveyed) == 0) {
    ind$ndessurcat <- NA
  } else {
    ind$ndessurcat <- calc_ndes(results[results$IsTarget == 1 & results$IsSurveyed == 1, ])
  }
  ...
  ind$invF <- ind$Surbio / ifelse(ind$Totcat <= 0, NA, ind$Totcat)
